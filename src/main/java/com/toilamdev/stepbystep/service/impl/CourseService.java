    ...
    public int createNewCourse(CourseRequestDTO courseRequestDTO) {
        try{
            ...
            if (currentUser.getAuthorities().stream().anyMatch(auth -> auth.getAuthority().equals("ROLE_USER"))
                ...
                throw new IllegalStateException("Bạn cần phải là Instructor để tạo khóa học");
            }

            Category category = this.categoryRepository.findByName(FormatUtils.formatName(courseRequestDTO.getCategory()))
                    .orElseThrow(
                            () -> new GlobalException.CategoryNotFoundException("Không tìm thấy Category phù hợp")
                    );

            Set<Tag> tags = courseRequestDTO.getTags().stream().map(tagName
                    -> tagRepository.findByName(FormatUtils.formatName(tagName)).orElseThrow(()
                    -> new GlobalException.TagNotFoundException("Không tìm thấy Tag phù hợp"))).collect(Collectors.toSet());

            Course newCourse = Course.builder()
                    .title(FormatUtils.formatName(courseRequestDTO.getCourseTitle()))
